...
public class RezerwacjaDaoImpl  extends AbstractDaoImpl<Rezerwacja> {
	...
	public void zatwierdzRezerwacje(Kurs k1, Kurs k2, String[] doRezerwacji, User user) {
		...
		List<Rozklad> r = getSession()
				...
				.add(Restrictions.sqlRestriction("trasa_info_id = "
						...
						+ " and dnikursu_id = "
						+ dk)).add(Restrictions.eq("numer", n)).list();
		...
		for (Rozklad rozklad : r) {
			kursy.addAll(getSession()
					...
					.add(Restrictions.sqlRestriction(" rozklad_id = " + rozklad.getId()
	...
	public List<Rezerwacja> getReservationsByUserId(long id) {
		return getSession().createQuery("FROM Rezerwacja R WHERE R.user = " + id).list();
	}
	
	public List<Integer> getSeatNumbersByReservationId(long id) {
		//return getSession().createQuery("FROM Rezerwacja R where").list();
		return getSession().createSQLQuery("select * from rezerwacja_miejscazarezerwowane where rezerwacja_id = " + id).list();
