			...
			<div class="modal-body">
				<form id="formEdit" action="users/update" method="POST">
					<div class="form-group">
						<label>Id: </label> <input class="form-control" id="id" type='text' name='id' readonly />
					</div>
					<div class="form-group">
						<label>Email: </label> <input class="form-control" id="email" type='text' name='email' />
					</div>
					<div class="form-group">
						<label>Imie: </label> <input class="form-control" id="firstName" type='text' name='firstName' />
					</div>
					<div class="form-group">
						<label>Nazwisko: </label> <input class="form-control" id="lastName" type='text' name='lastName' />
					</div>
					<div class="form-group">
						<label>Numer telefonu: </label> <input class="form-control" id="phoneNumber" type='text' name='phoneNumber' />
					</div>
					<div class="form-group">
						<label>Numer dokumentu potwierdzającego: </label> <input class="form-control" id="idCardNumber" type='text' name='idCardNumber' />
					</div>
			...
			<div class="panel-body">
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-hover" id="dataTables-example">
						...
						<tbody>
							<c:forEach items="${pracownicy}" var="u">
								<tr class="even gradeC" id="${u.id}">
									<td class="id">${u.id}</td>
									<td class="email">${u.email}</td>
									<td class="firstName">${u.firstName}</td>
									<td class="lastName">${u.lastName}</td>
									<td class="phoneNumber">${u.phoneNumber}</td>
									<td class="idCardNumber">${u.idCardNumber}</td>
									<td class="idCardNumber"><c:forEach items="${u.roles}" var="r">${r.name}<br>
										</c:forEach></td>
									<td id="test2"><a class="btn-sm btn-warning btn-primary edit" id="">Edytuj</a> <a class="btn-sm btn-danger remove">Usuń</a></td>
								</tr>
...
<script>
	...
	function postData() {
		...
		if ($("#firstName").val() === $('#' + $("#id").val())
			...
			alert("Wyedytowałeś dane Pracownika o id = " +  $("#id").val());
			form.submit();
		} */
